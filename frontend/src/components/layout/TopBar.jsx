...
export default function TopBar() {
  ...
  const timeSince = lastUpdate
    ? ((Date.now() - new Date(lastUpdate).getTime()) / 1000).toFixed(1) + 's ago'
    : '—';

  return (
    <header className="fixed top-0 left-56 right-0 h-12 z-30 flex items-center px-5 gap-4
        ...
        >
          <span
            className={`status-dot ${
              selected?.status === 'healthy' ? 'status-dot-live' :
              selected?.status === 'monitoring' ? 'status-dot-warning' : 'status-dot-muted'
            }`}
          />
        ...
        {open && (
          <div className="absolute top-full mt-1 left-0 w-56 card py-1 z-50">
            {stations.map(s => (
              <button
                key={s.id}
                onClick={() => { setSelectedStation(s.id); setOpen(false); }}
                className={`w-full flex items-center gap-2.5 px-3 py-2 text-sm text-left
                           hover:bg-atmo-mid transition-colors
                           ${s.id === selectedStation ? 'text-teal font-semibold' : 'text-atmo-deep'}`}
              >
                <span className={`status-dot ${
                  s.status === 'healthy' ? 'status-dot-live' :
                  s.status === 'monitoring' ? 'status-dot-warning' : 'status-dot-muted'
                }`} />
                <div>
                  <div className="font-medium">{s.id}</div>
                  <div className="text-2xs text-atmo-muted">{s.location.name}</div>
                </div>
              </button>
            ))}
          </div>
      ...
      <div className="flex items-center gap-1.5 text-xs">
        <span className={`status-dot ${
          connectionStatus === 'CONNECTED' ? 'status-dot-live' :
          connectionStatus === 'POLLING'   ? 'status-dot-warning' : 'status-dot-muted'
        }`} />
        <span className="text-atmo-muted font-medium">
          {connectionStatus === 'CONNECTED' ? 'WebSocket' :
           connectionStatus === 'POLLING'   ? 'Polling'   : connectionStatus} ●{' '}
      ...
      <button className="relative p-1.5 rounded-lg hover:bg-atmo-mid transition-colors">
        ...
        {state.anomalyStats.high > 0 && (
          <span className="absolute -top-0.5 -right-0.5 w-3.5 h-3.5 rounded-full bg-critical
